    ...
    try {
        ...
        const context = {
            ...
            profileName: (() => {
                ...
                const emailFrom = (path) => {
                    try {
                        return JSON.parse(readFileSync(path, "utf-8"))?.oauthAccount?.emailAddress || null;
                    }
                    catch {
                        return null;
                    }
                };
                const liveEmail = emailFrom(join(dir, ".claude.json"));
                if (liveEmail)
                    return liveEmail;
                try {
                    ...
                    for (const name of backups) {
                        const email = emailFrom(join(backupsDir, name));
                        if (email)
                            return email;
                    }
                }
