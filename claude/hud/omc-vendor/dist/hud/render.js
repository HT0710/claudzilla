...
export async function render(context, config) {
    ...
    if (enabledElements.contextBar) {
        const ctx = enabledElements.useBars
            ? renderContextWithBar(context.contextPercent, config.thresholds, 10, context.contextDisplayScope, hudLabels, context.contextWindowSize, context.contextUsedTokens)
            : renderContext(context.contextPercent, config.thresholds, context.contextDisplayScope, hudLabels, context.contextWindowSize, context.contextUsedTokens);
        if (ctx)
